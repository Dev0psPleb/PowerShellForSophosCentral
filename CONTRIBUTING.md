# PowerShell For Sophos Central PowerShell Module
## Contributing
...
- [PowerShell For Sophos Central PowerShell Module](#powershell-for-sophos-central-powershell-module)
...
it grow.  There are many different ways that you can contribute:
 ...
 5. Help answer [questions](https://github.com/ralph-brynard/PowerShellForSophosCentral/issues?q=is%3Aopen).
...
PowerShell For Sophos Central team via [Issues](https://github.com/ralph-brynard/PowerShellForSophosCentral/issues), and ensure it
follows the product roadmap. Note that all code submissions will be rigorously reviewed by the
PowerShell For Sophos Central Team. Only those that meet a high bar for both quality and roadmap fit will be merged
into the source.

## Maintainers

PowerShell For Sophos Central is maintained by:

- **[@ralph-brynard](http://github.com/ralph-brynard)**

The only workflow restrictions are that a member above must approve a pull-request before the committed changes will be merged into either the dev or main branches:
...
All issue types are tracked on the project's [Issues](https://github.com/ralph-brynard/PowerShellForSophosCentral/issues)
page.
...
* The version of the module (located in `PowerShellForSophosCentral.psd1`)
* Your OS version
...
This is a manifested PowerShell module, and the manifest can be found here:
    ...
    PowerShellForSophosCentral.psd1
 ...
 * In `Import-SophosCentralConfiguration`, update `$config` to declare the new property along with
   its default value, being sure that PowerShell will understand what its type is. Properties
   should be alphabetical.
 * Update `Get-SophosCentralConfiguration` and add the new property name to the `ValidateSet` list
 ...
 * Add a new explicit parameter to `Set-SophosCentralConfiguration` to receive the property, along with
...
```powershell
Set-SophosCentralConfiguration -LogRequestBody
```
